...
async function deleteUserAccount(userId) {
    ...
    const { error: promoError } = await supabaseClient
        ...
        .delete()
        .eq('user_id', userId);
    if (promoError) console.error('Error deleting promo redemptions:', promoError);
...
// =====================================
// SUBSCRIPTION FUNCTIONS (Mobile Paywall)
// =====================================
...
async function updateSubscriptionStatus(userId, status, purchaseToken, expiresAt) {
    if (!supabaseClient) return false;
...
async function redeemPromoCodeRPC(code) {
    if (!supabaseClient) throw new Error('Supabase not initialized');

    const { data, error } = await supabaseClient
        .rpc('redeem_promo_code', { code_input: code });
    ...
    window.redeemPromoCodeRPC = redeemPromoCodeRPC;
